...
public class JobInfoFrame extends BaseFrame{
	...
	public JobInfoFrame() {
		...
		north.add(createLable(new JLabel("채용정보"), new Font("HY헤드라인M",1,24)));
		...
		var btnSubmit = createButton("지원하기", e->{
			executeSQL("INSERT INTO applicant VALUES(0,?,?,0)", dtm.getValueAt(table.getSelectedRow(),0),Session.userNo);
			iMsg("신청이 완료되었습니다.");
			updateList(false);
		});
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				frame.btnCheck.addActionListener(e2 ->{
					...
					if(!eGender.equals("무관") && !eGender.equals(gender[Session.userGender])) {
						eMsg("지원이 불가합니다. (성별)");
						return;
					} 
					
					if(!eGraduate.equals("무관") && Session.userGraduate > Arrays.asList(grad).indexOf(eGraduate)) {
						eMsg("지원이 불가합니다. (학력)");
						return;
					}
