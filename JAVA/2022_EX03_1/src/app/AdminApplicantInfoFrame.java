...
public class AdminApplicantInfoFrame extends BaseFrame {
	...
	public AdminApplicantInfoFrame() {
		...
		JMenuItem[] menuItems = { new JMenuItem("합격"), new JMenuItem("불합격") };
		...
		try {
			...
			while (rs.next()) {
				...
				panel.add(createcomp(
						createLable(new JLabel("지원 회사 : " + rs.getString("c_name")), new Font("HY헤드라인M", 1, 18))
						, 160, 10, 280, 30));
				
				int age = 10;
				
				panel.add(createcomp(
						createLable(new JLabel("이름 : " + rs.getString("u_name") + "(나이 : " + age + "세)"), new Font("굴림", 1, 14))
						, 170, 40, 280, 30));
				
				panel.add(createcomp(
						createLable(new JLabel("생년월일 : " + rs.getString("u_birth")), new Font("굴림", 1, 14))
						, 170, 70, 280, 30));
				
				panel.add(createcomp(
						createLable(new JLabel("최종학력 : " + grad[rs.getInt("u_graduate")]), new Font("굴림", 1, 14))
						, 170, 100, 280, 30));
				
				panel.add(createcomp(
						createLable(new JLabel("이메일 : " + rs.getString("u_email")), new Font("굴림", 1, 14))
						, 170, 130, 280, 30));
